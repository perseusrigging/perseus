...
static const MString MEL_COMMAND = "prSimplePairBlend";
MString prSimplePairBlendCmd::nodeName;

prSimplePairBlendCmd::prSimplePairBlendCmd() :
    MPxCommand()
{
}
...
MStatus prSimplePairBlendCmd::doIt(const MArgList& args)
{
    MStatus status;
    return(redoIt());
}

MStatus prSimplePairBlendCmd::undoIt()
{
    mDGModifier.undoIt();
    mDagModifier.undoIt();
    return(MS::kSuccess);
...
MStatus prSimplePairBlendCmd::redoIt()
{
    MStatus status;
    ...
    if (selectList.length() == 1)
    {
        ...
        nodeName = MString("prSimplePairBlend_") + srcDag.name();
        mShapeObj = mDGModifier.createNode(prSimplePairBlend::GetTypeName());
        mDGModifier.doIt();
...
bool prSimplePairBlendCmd::isUndoable() const
{
    return(true);
}

void* prSimplePairBlendCmd::Creator()
{
    return(new prSimplePairBlendCmd());
}

MString prSimplePairBlendCmd::CommandName()
{
    return(MEL_COMMAND);
}

MSyntax prSimplePairBlendCmd::CreateSyntax()
{
    MSyntax syntax;
    return(syntax);
}
